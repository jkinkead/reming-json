...
class CollectionFormatsSpec extends BaseSpec {
  import DefaultJsonProtocol._

  "listFormat" should "serialize a List[Int] to an array" in {
    PrettyPrinter.printToString(List(1, 2, 3)) shouldBe "[1, 2, 3]"
  }
  it should "read a serialized int array" in {
    JsonParser.read[List[Int]]("[1,2,3]") shouldBe List(1, 2, 3)
  }

  "arrayFormat" should "serialize an Array[Int] to an int array" in {
    PrettyPrinter.printToString(Array(1, 2, 3)) shouldBe "[1, 2, 3]"
  }
  it should "read a serialized int array" in {
    JsonParser.read[Array[Int]]("[1,2,3]") shouldBe Array(1, 2, 3)
  }

  "stringMapFormat" should "serialize a Map[String, Long] to an object" in {
    val map = Map("a" -> 1, "b" -> 2, "c" -> 3)
    PrettyPrinter.printToString(map) shouldBe """{
        |  "a": 1,
        |  "b": 2,
        |  "c": 3
        |}""".stripMargin
  ...
  "seq format" should "serialize a seq of objects" in {
    ...
    PrettyPrinter.printToString(seq) shouldBe """[{
        |  "a": "a"
        |}, {
        |  "a": "b",
        |  "b": 1
